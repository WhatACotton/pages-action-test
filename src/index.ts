...
try {
	...
	const setProductionBranch = async (): Promise<{ before_branch: string, current_branch: string }> => {
		...
		return json.result.latest_deployment.production_branch, json.result.production_branch
	}
...
| **Branch Preview URL**: | ${aliasUrl ? aliasUrl : "This URL was not generated because the branch is the production branch."} |
	...
	(async () => {
		...
		let alias = "";
		console.log("branch:", branch, "prod_branch:", production_branch);
		...
		if (before_branch !== current_branch) {
			summary.addRaw(`converting production branch from ${before_branch} to ${current_branch} successfully!`).write();
		}
	})();
} catch (thrown) {
